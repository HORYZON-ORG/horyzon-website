...
export function OperatingJourney({ compact = false }: { compact?: boolean }): React.JSX.Element {
 return <div className={`method-chapters ${compact ? 'is-compact' : ''}`}>
  ...
  <div className="method-chapter-list">{chapters.map((chapter,index)=><details key={chapter.title} open={index===0}>
   <summary><span className="method-chapter-number">0{index+1}</span><span><strong>{chapter.title}</strong><small>{chapter.text}</small></span><span className="method-chapter-toggle" aria-hidden="true">+</span></summary>
