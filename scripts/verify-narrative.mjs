...
for (const phrase of ['Benessere organizzativo','Benessere patrimoniale','Benessere digitale']) assert(system.includes(phrase), `le-tre-aree: missing ${phrase}`);
...
for (const phrase of ['Inizia il Radar','Parliamo del risultato','Raccontaci che cosa ti blocca']) assert(contact.includes(phrase), `contatti: missing ${phrase}`);
assert(home.includes('<title>Horyzon — Benessere per l’impresa</title>'), 'home: narrative title missing');
for (const phrase of ['radar d’impresa', 'benessere organizzativo', 'benessere patrimoniale', 'benessere digitale']) assert(home.toLowerCase().includes(phrase), `home metadata: missing ${phrase}`);
